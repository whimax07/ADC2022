...
class Graph:
    ...
    def compute_all_permutations(self):
        ...
        for i in range(3, int(len(valid_valves)/2)+1):
            changed = False
            for moves in tqdm(itertools.permutations(valid_valves, i)):
        ...
        print('Computing best unique move combination...')

        best_score = 0
        all_moves = list(self.scores.keys())
        set_moves = [set(k) for k in all_moves]
        for i in tqdm(range(0, len(set_moves))):
            for j in range(i+1, len(set_moves)):
                if set_moves[i] & set_moves[j]:
                    continue
                best_score = max(best_score, self.scores[all_moves[i]] + self.scores[all_moves[j]])
